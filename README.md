...
- **Supabase 공식 문서 모범 사례 준수** (`@supabase/ssr` 사용)
- 환경별 Supabase 클라이언트 분리:
  - Client Component용 (`useClerkSupabaseClient` - `@supabase/ssr`의 `createBrowserClient`)
  - Server Component용 (`createClerkSupabaseClient` - `@supabase/ssr`의 `createServerClient`)
  - 관리자 권한용 (`getServiceRoleClient`)
  - 공개 데이터용 (`supabase` - `@supabase/ssr`의 `createBrowserClient`)
- SQL 마이그레이션 시스템
- 쿠키 기반 세션 관리 (SSR 최적화)
...
6. 다음 정보 입력:
   ...
   - **JWT Issuer (Issuer URL)** 또는 **Domain**:
     ...
     ```
     (3-1 단계에서 복사한 Clerk domain으로 교체)
...
> **참고**: 이 통합을 통해 Clerk 세션 토큰에 `"role": "authenticated"` 클레임이 자동으로 추가되어 Supabase RLS 정책에서 인증된 사용자로 인식됩니다.
...
**테스트 페이지:**
- `/instruments`: Supabase 연결 테스트 (공식 문서 예시 기반)
...
프로젝트에 이미 Clerk 한국어 로컬라이제이션이 적용되어 있습니다. `app/layout.tsx`의 `ClerkProvider`에서 `koKR` locale이 설정되어 있으며, 커스텀 에러 메시지도 포함되어 있습니다.

**주요 특징:**
- 기본 한국어 로컬라이제이션 (`koKR`) 사용
- 커스텀 한국어 에러 메시지 추가
- `html lang="ko"` 설정으로 접근성 향상

**로컬라이제이션 커스터마이징:**

더 자세한 내용은 [Clerk 로컬라이제이션 가이드](./docs/CLERK_LOCALIZATION.md)를 참고하세요.
...
프로젝트에 RLS 정책 예시 마이그레이션 파일이 포함되어 있습니다:
- `supabase/migrations/20250101000000_example_rls_policies.sql`

이 파일은 Clerk + Supabase 통합 시 사용할 수 있는 완전한 RLS 정책 예시를 제공합니다.

**추가 테이블 생성 시 RLS 정책 설정:**
...
- 더 자세한 예시는 `supabase/migrations/20250101000000_example_rls_policies.sql` 파일을 참고하세요
...
- [프로젝트 TODO](./docs/TODO.md) - 개발 계획 및 진행 상황
